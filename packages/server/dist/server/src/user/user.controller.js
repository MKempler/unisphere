...
let UserController = class UserController {
    ...
    async followUser(handle, req) {
        const result = await this.userService.followUser(req.user.id, handle);
        if (!result.ok) {
            throw new common_1.BadRequestException(result.error);
        }
        return api_response_1.ApiResponse.success({ success: result.data });
    }
};
exports.UserController = UserController;
__decorate([
    (0, common_1.Get)('profile/:handle'),
    (0, swagger_1.ApiOperation)({ summary: 'Get a user profile by handle' }),
...
__decorate([
    (0, common_1.Post)('follow/:handle'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, swagger_1.ApiBearerAuth)(),
    (0, swagger_1.ApiOperation)({ summary: 'Follow a user' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Follow request processed' }),
